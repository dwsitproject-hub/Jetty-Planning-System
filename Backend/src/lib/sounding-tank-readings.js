...
function normalizeReadingSide(item, measurementBasis, sideLabel) {
...
export function validateAndNormalizeTankReadings(raw, opts = {}) {
  ...
  for (const item of raw) {
    ...
    if (captureMode === 'dual') {
      const atg = normalizeReadingSide(item.atg, measurementBasis, 'ATG');
    ...
    const side = normalizeReadingSide(item, measurementBasis, captureMode === 'manual' ? 'Manual' : 'ATG');
    ...
    out.push({
      ...
      lockedAt,
      stabilization:
        item.stabilization && typeof item.stabilization === 'object'
          ? item.stabilization
          : {
              state: captureMode === 'manual' ? 'manual' : 'locked',
            },
      atgSourceBaseUrl: item.atgSourceBaseUrl ?? item.atg_source_base_url ?? null,
      sessionId: item.sessionId ?? item.session_id ?? null,
    });
