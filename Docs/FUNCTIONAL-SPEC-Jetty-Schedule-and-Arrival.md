...
| **Multi-jetty — spanning bar (Gantt)** | When a row carries **`additionalJetties`**, the **Actual** (or **Planned**) bar renders in the **primary jetty’s lane** and a **span extension** overlay continues through the **same bank lane index** on each adjacent jetty row (e.g. **2B-01** + **3B-01**). Adjacent jetty rows are **interleaved** by lane (`2B-01, 3B-01, 2B-02, 3B-02`) so the overlay never crosses an unrelated lane. Primary and extension pieces share fill/border and move together on drag. **Late (past ETC)** bars show one continuous **3px red left accent** on the full combined height (outer shell on both pieces). Left id-column status on a **spanned** lane reads e.g. *Occupied (1/2) · spanned by &lt;vessel&gt;*; a **non-spanned** lane on the same jetty may still read *Available · jetty Occupied (1/2)*. See **§2.28**. |
...
| **Jetty schematic** | Stacks for **Out of Service** berths are **muted**, show an **OOS** badge, and tooltips state the jetty is **not available for new allocation**. When **View as of = today**, schematic **occupied** lane count aligns with **Slot occupancy** (same **`scheduleQueue`** alongside rules; excludes **SAILED**, **shifting-out**, and **OOS** denominator jetties). **Multi-jetty berthing:** a vessel spanning into a secondary jetty reserves that jetty’s **matching lane index**; schematic shows a **spanned** placeholder on the secondary lane and may widen the primary vessel graphic across columns (**§2.28**, **§17.6–17.8**). |
...
| **Adding SIs after approval or during operations** | **Normal UI — not supported:** On **`/shipment-plans/:id`**, **Add SI**, **Edit plan**, and **Submit for approval** are shown only when the plan **`approval_status`** is **Draft** or **Rejected**. On **Approved** or **Submitted** plans those controls are hidden; the plans list **Edit** icon opens the combined **Update shipment plan** modal when pre-berth (same form as create/edit, but **Add another SI** is hidden and only existing SIs are saved). **Pre-berth combined edit:** **`PATCH /shipment-plans/:id`** updates the plan shell while **no operation on the plan has recorded TB** — ETA, preferred jetty, purpose, voyage, agent, and vessel fields — **without** reopening approval. **Linked plans** (**§2.30**): vessel name/LOA/GT/draft change only by **re-picking Master vessel** (dimensions read-only in the modal). **Legacy plans**: free-text vessel name/LOA/GT/draft as before. DWT remains derived. Existing child SIs are updated in the same modal via **`PUT /shipping-instructions/:id`** (**§2.13** SI rules below). **Post-berth:** plan shell update returns **409**. **Vessel Info** modal (click vessel name on Allocation / plans list) follows the same linked vs legacy rules via **`PATCH …/vessel-info`** in any approval status. See pre-berth SI edit rules below. **Intended workflow:** define all SIs before submit/approve, or use the **late SI** path (plan + arrival first, SI later — **§2.14**). **API exception:** **`POST /shipping-instructions`** with **`shipment_plan_id`** still creates a new SI on an **Approved** or **Submitted** plan and is **not** blocked when **`operations`** already exist. The plan is **reopened** to **Draft**, all existing child SIs reset to **Draft**, and the response includes **`planReopened: true`**. In-flight **`operations`**, allocation timestamps, and at-berth work are **not** deleted, but **Confirm Berthing** stays blocked until the plan is **re-approved** (**§2.14**). **Pre-berth SI edit (Approved/Submitted):** **`PUT /shipping-instructions/:id`** is allowed while **no operation on the plan has recorded TB** (pre-berth). **Cosmetic / late-fill** fields — **`reference_number`**, **`note`**, **`document_date`** — save without reopening the plan (plan stays **Approved**). **Material** fields — breakdown (commodity, qty, shipper, contract/PO/SO), trade term, ports, surveyor, preferred jetty, Loading-only text fields — save but **reopen** the plan to **Draft** (same semantics as POST new SI; response includes **`planReopened: true`**). **Submitted** plans: material edits are blocked until an approval decision; cosmetic fields remain editable. **Post-berth:** any SI update returns **409** (*Shipping instruction cannot be edited after berthing.*). UI entry point: **Update shipment plan** on the allocation queue when berthing is blocked; **Update shipment plan** on **`/shipment-plans/:id`** hub header; plans list edit icon on Approved pre-berth rows. Deep link **`/shipment-plans?shipment_plan_id=<id>`** opens the full plan edit modal only for **Draft/Rejected**; for Approved pre-berth it opens the combined **Update shipment plan** modal. |
...
| **Late SI (plan before SI)** | **Shipment plans** may be created with **vessel + ETA + purpose** only (no SI). Plans with **zero SIs** appear in **`/allocation-plans`** as **`source`** **`incoming-plan`**. **Log arrival update** accepts **`shipmentPlanId`** alone (jetty, ETA, ETB, priority, remark, No PKK on the plan — not TA/TB/ETC). **Shipping Instructions No.** on a child SI is **optional at create** (cargo/details may be saved while the official number is pending). **Berthing** is **disabled** until the **shipment plan** is **`Approved`**, has **at least one** shipping instruction, and every SI has a **valid reference number** — not blank, not a single quote (`'`), and not the same as the vessel name (case-insensitive); tooltip and API reject alongside timestamps until then. Approving a plan cascades **`Approved`** to all child SIs; adding a new SI to an **Approved** or **Submitted** plan **reopens** the plan to **Draft** for re-approval. **Pre-berth combined edit:** on **Approved/Submitted** plans with **no TB recorded** on any plan operation, operators open **Update shipment plan** from the allocation queue (when berthing is blocked), plan hub, or plans list — one modal edits the plan shell and existing SIs together. Cosmetic SI updates (especially **SI reference number**) keep the plan **Approved** and can unlock berthing; material SI changes reopen the plan (**§2.13**). **Confirm Berthing** (TA, TB, ETC, vessel photo) uses the existing **Berthing** modal after the gate opens. |
...
| **Where the berth hours go** | Waterfall of **average** phase durations on sailed voyages in period: anchorage wait, pre-checking, cargo operations, idle/delays at berth, post-checking & sign-off. Each segment is the **arithmetic mean of the same per-voyage value** shown in voyage drill-down expansion and waterfall segment modals (not a residual of fleet-average berth minus other averages). **Idle / delays at berth** = mean of `berth − pre − cargo-ops − post` per voyage (missing phases count as 0 inside idle only; **post** capped at berth via **`postH`**). **Post-checking** uses **`postH`** (excluded when duration exceeds berth). Sub-process outliers (`pre` or `post` &gt; berth) are nulled in row normalization. Berth-side segment averages may not sum exactly to average berth time when phase logging coverage differs across voyages. Caption shows average port stay (sum of segment averages) and cargo-work share. |
...
### 2.28 Multi-jetty berthing

**Purpose:** Some physical moorings span more than one jetty column (e.g. a vessel moored across **2B** and **3B**). When enabled for a port, planners assign a **primary jetty** plus optional **additional (adjacent) jetties** on the same vessel call. The system treats the call as occupying the primary jetty’s bank lane **and** the **same lane index** on each spanned jetty, while other lanes on those jetties may remain available.

**Configuration (admin):**

| Area | Behaviour |
|------|------------|
| **Master – Port** | Checkbox **Allow Multi-Jetty Berthing** on add/edit port (`allow_multi_jetty_berthing`). Default **off**. When off, Allocation hides additional-jetty controls and the API rejects non-empty **`additionalJetties`**. |
| **Master – Preferred Jetty** | Multi-select **Adjacent Jetties** — explicit pairs stored in **`jetty_adjacencies`** (symmetric). **Independent** of **Jetty Layout** schematic positioning; admins pick jetties on the **same mooring line**, not jetties separated only by the pipeline graphic. Adjacency can be configured even before the port flag is enabled (for pre-setup). List column **Adjacent Jetties** shows resolved short ids. |
| **Port session** | Active port exposes **`allowMultiJetyBerthing`** on the port scope payload so Allocation modals know whether to show the picker. |

**Allocation — berthing UI:**

| Surface | Behaviour |
|---------|------------|
| **Confirm Berthing**, **Log arrival update**, **Active Vessel Detail (edit)** | When the port flag is on and a **primary jetty** is selected, **`JettyAllocationSelect`** shows an **Additional jetties (multi-jetty berthing)** checkbox list of jetties **adjacent** to the primary. Each option shows occupancy hints (**Occupied**, **Occupied (n/capacity)**, **✗ not suitable**). A jetty is **disabled** only when **fully** occupied (every bank lane taken, including spanned lanes) or fails jetty advice — a double-bank jetty at **1/2** because one lane is spanned remains selectable for span into the free lane. Empty adjacency config shows a hint to configure **Master – Jetty**. |
| **Save** | **`PUT /allocation/arrival`** accepts **`additionalJetties`**: array of short jetty ids (e.g. **`["3B"]`**). Omitted key leaves stored values unchanged; **`[]`** clears additional jetties. Persisted on **`shipment_plans.additional_jetties`** and mirrored on **`operations.additional_jetties`**. |
| **Validation (server)** | Port flag must be on; each additional jetty must be **adjacent** to the primary; primary cannot appear in the additional list; additional jetties must not be **Out of Service**; at least one selected additional jetty must support the operation’s **commodity type** for its **purpose** when commodity metadata exists; no **active** occupant (primary or secondary) on the target additional jetty unless excluded (current operation). Failures return **400** or **409** with a plain-language message. |

**Occupancy model:**

| Concept | Behaviour |
|---------|------------|
| **Primary berth** | Vessel occupies its assigned **bank lane** on the primary jetty (same double-bank rules as **§2.1**). |
| **Spanned lanes** | Each additional jetty reserves **one lane**: the **same lane index** as the primary bar, clamped to that jetty’s **capacity**. |
| **`berths[].occupiedCount`** | **Direct occupants** on that jetty **plus** lanes spanned into from adjacent primaries (e.g. **1/2** when one lane is spanned, one is free). |
| **`berths[].spannedByLanes`** | Per-lane list `{ laneIndex, primaryBerthId, vesselId, vesselName }` for lanes held by a spanning vessel from another jetty. Legacy **`spannedBy`** = first entry. |
| **Blocking operations** | **Out of Service** and master blocking counts treat a jetty as in use when it is primary **or** listed in any active operation’s **`additional_jetties`**. |
| **Shifted out** | Unchanged: **`shifting_out`** rows do not occupy primary or spanned capacity. |

**Jetty schedule (Gantt):**

| Area | Behaviour |
|------|------------|
| **Row order** | Adjacency-linked jetties use **lane-interleaved** rows (`2B-01`, `3B-01`, `2B-02`, `3B-02`) so a spanning bar’s primary and extension rows are always adjacent. |
| **Spanning overlay** | Real bar (text, drag, click) stays on the **primary** row; a **span extension** sibling fills additional jetty rows for the same time range. Shared fill/border; seamless top/bottom join between pieces; **unified drag** from either piece. Extension horizontal position follows the **measured** primary bar rect (pixel-aligned). |
| **Late accent** | **ETC overdue** spanning bars: **3px red left accent** on **both** shell pieces via outer **`box-shadow: inset`**; inner duplicate on the primary row suppressed so the stripe reads as one continuous line. |
| **Status column** | Per-row labels distinguish direct occupancy vs **spanned by &lt;vessel&gt;** vs **Available · jetty Occupied (n/cap)** on a free lane of a partially occupied jetty. |

**Jetty schematic:**

| Area | Behaviour |
|------|------------|
| **Primary column** | Vessel graphic may **widen** across adjacent berth columns when **`additionalJetties`** span horizontally. |
| **Secondary lane** | Matching lane shows a **spanned** placeholder (no duplicate vessel card) with tooltip *spanned by &lt;vessel&gt;*. |
| **Occupancy parity** | Schematic lane assignment uses the same **TB → operation id → vessel id** ordering and **`spannedByLanes`** metadata as overview **`berths`** / **`jettyScheduleOccupancy.js`**. |
...
### 2.30 Master vessel link on shipment plans (snapshot model)

**Purpose:** Tie new shipment plans to canonical vessel master data while preserving historical free-text plans created before this feature. Plan columns **`vessel_name`**, **`vessel_loa_m`**, **`vessel_gross_tonnage`**, and **`vessel_draft`** are **copied snapshots** — master renames or dimension corrections do **not** retroactively change existing linked plans (future “push to plans” is deferred — **`Docs/Future/MASTER-VESSEL-PUSH-TO-PLANS.md`**).

#### 2.30.1 Linked vs legacy

| Mode | **`master_vessel_id`** | How vessel fields are edited |
|------|------------------------|------------------------------|
| **Linked** | Set (FK to **`master_vessels`**) | **Re-pick Master vessel** only — LOA, GT, and draft are read-only in UI and refreshed from master on re-pick. Direct free-text edits to name/LOA/GT/draft are rejected by the API. |
| **Legacy** | **NULL** | Free-text **vessel name**, **LOA**, **GT**, and **draft** as before integration with master data. No master dropdown. |

API list/detail responses expose **`masterVesselId`** and **`vesselLinkStatus`**: **`linked`** or **`legacy`**.

#### 2.30.2 Create and edit surfaces

| Surface | Behaviour |
|---------|-----------|
| **Create shipment plan** (`/shipment-plans` → **Create**) | **Master vessel** dropdown is **required** (active **`master_vessels`** for the port). Selecting a master auto-fills **vessel name**, **LOA (m)**, **GT**, and **draft** (read-only). Optional **DWT / capacity** may still be entered separately. **`POST /shipment-plans`** returns **400** without **`master_vessel_id`**. |
| **Update shipment plan** (pre-berth combined modal) | **Linked** plan: master dropdown shown; changing selection updates the snapshot. **Legacy** plan: free-text vessel fields unchanged from prior behaviour. |
| **Vessel Info modal** (vessel name link on Allocation, At-Berth, Clearance, Shipment plans) | Same dual-mode rules via **`PATCH /shipment-plans/:id/vessel-info`**, allowed in **any** approval status. Linked: master re-pick only. Legacy: free-text fields. |
| **Plans list** (`/shipment-plans`) | Rows with **`vesselLinkStatus === 'legacy'`** show a **Legacy** badge beside the vessel name so operators can distinguish pre-migration / unlinked plans. |

#### 2.30.3 Partner API and implicit plan create

| Path | Behaviour |
|------|-----------|
| **`POST /api/v1/integrations/shipping-instructions`** | Resolves master by **`vessel_hub_code`** (preferred) or unique **`vessel_name`**. Created plan is **linked** with snapshot vessel fields. See **§2.23**. |
| **`POST /shipping-instructions`** (no **`shipment_plan_id`**) | Implicit plan create requires **`master_vessel_id`** in the request body. |

#### 2.30.4 Master vessel delete guard

Soft-deleting a **Master vessel** (**`/master/vessel`**) is blocked (**409**) when one or more non-deleted **shipment plans** still reference that **`master_vessel_id`**.

#### 2.30.5 Rollout notes

| Item | Behaviour |
|------|-----------|
| **Migration** | **118** adds **`master_vessel_id`** with **no backfill** — existing plans remain **legacy**. |
| **Deploy** | Run migration **118** and **rebuild/redeploy the API** (not restart-only) before UAT — stale API images ignore **`master_vessel_id`** on create. |
| **Deferred backfill** | Linking legacy plans to master in bulk is **Project B** — not part of go-live. |
...
### 2.31 System Health Dashboard (infrastructure health)
...
| Area | Behaviour |
|------|------------|
| **Access** | **Admin hub → System Health Dashboard** (`/admin/operations`). Same **`admin`** page permission as User Management, Notifications, Partner API keys. No operational port selection required. |
...
| Primary + additional jetties | **`jetty`** (short id) = primary berth; **`additionalJetties`** = array of short ids for adjacent spanned jetties (empty when feature off or single-jetty call). Drives Gantt span overlay and schematic **`spannedByLanes`**. |
...
| **Request body (relevant)** | Includes `estimatedCompletionDateTime`, optional **`additionalJetties`** (short jetty id array — **§2.28**). **`actualCompletionDateTime`** is accepted only when the targeted operation is already **`SAILED`**; otherwise the API **ignores** client edits and keeps the stored value (depart is the write path for actual completion). |
...
| **Overview fields** | `GET /allocation/overview` queue rows include **`shipmentPlanId`**, **`additionalJetties`**, **`operationsCompletedDateTime`**, **`actualCompletionDateTime`**, **`castOffDateTime`**, **`recordLastUpdatedAt`** and **`recordLastUpdatedByDisplayName`** derived from **`GREATEST(shipment_plans.updated_at, operations.updated_at)`** (and SI `updated_at` for incoming rows without an operation). **`berths[]`** includes **`occupiedCount`**, **`spannedByLanes`**, and legacy **`spannedBy`** for multi-jetty occupancy (**§2.28**). Operation-backed rows also include **`jettyOperationCode`** when migration **056** is applied (**§2.10**). |
...
| Area | Location |
|------|----------|
| Gantt UI & segment logic | `Frontend/src/components/JettyScheduleGantt.jsx` — includes multi-jetty span overlay, adjacency-aware row order (**§2.28**) |
| Multi-jetty berthing (adjacency, span UI, occupancy) | **§2.28** — migrations **095**, **096**; `Backend/src/lib/multi-jetty.js`, `Backend/src/routes/allocation.js`, `Backend/src/routes/jetties.js`, `Backend/src/routes/ports.js`, `Backend/src/lib/jetty-blocking.js`; `Frontend/src/components/JettyAllocationSelect.jsx`, `JettyScheduleGantt.jsx`, `JettySchematic.jsx`, `Frontend/src/utils/jettyAdjacency.js`, `Frontend/src/utils/jettyScheduleOccupancy.js`, `Frontend/src/utils/allocationPlanPovMerge.js`, `Frontend/src/pages/MasterPort.jsx`, `Frontend/src/pages/MasterJetty.jsx`; TECH-SPEC **§0.34** |
...
| Jetty Schematic (layout, **View as of**, date-filtered occupancy, lane content / purpose–RAG styling) | `Frontend/src/components/JettySchematic.jsx`, `Frontend/src/styles/jetty-schematic.css`, `Frontend/src/components/PurposeBadge.jsx`, `Frontend/src/components/EtcBreachBadge.jsx` (**ETC icon** in **`jetty-card__title-actions`** beside purpose badge); schematic **`materialDisplay`** / **`ragStatus`** on `Frontend/src/pages/Allocation.jsx` (`schematicMaterialDisplay`, `getEtcBreachRagStatus`) |
...
| **System Health Dashboard (infrastructure health & email alerts)** | **§2.31** — `Backend/src/lib/admin-ops-checks.js`, `admin-ops-partner-api-check.js`, `admin-ops-alert-job.js`, `Backend/src/routes/admin-ops.js`, `Backend/scripts/run-admin-ops-alerts.js`, `Backend/scripts/check-synology-mount.sh`; migration **`118_admin_ops_alert_settings.sql`**; `Frontend/src/pages/AdminOperations.jsx`, `Frontend/src/api/adminOps.js`; guides **`Docs/Guide/ADMIN-OPS-EMAIL-ALERTS.md`**, **`Docs/Guide/SYNOLOGY-MOUNT-TROUBLESHOOTING-AND-RECOVERY.md` §10**. TECH-SPEC **§0.36**. |
...
### 17.5 Relationship to double bank and multi-jetty berthing

Double-banking (multiple vessels per jetty, schedule `01`/`02` lanes, one-vessel-per-box schematic) is documented and implemented separately. **Multi-jetty berthing** (**§2.28**) adds spanning across **adjacent jetty columns** on the **same bank lane index**; it composes with double bank (e.g. **2B-01** + **3B-01** while **2B-02** and **3B-02** may stay free). Jetty **layout** only controls **which jetty** sits in which **schematic cell**; **capacity**, **Out of Service** status, **adjacency**, and **CCTV** links still come from overview **`berths`** / master data. **Occupants** shown in lane boxes are **derived from `scheduleQueue`** for the selected **View as of** day (**§17.8**), not from live **`berths.occupants`**. **Shifted-out** rows are excluded from schematic occupancy (`shifting_out`); they may still appear as **incoming** hints when date rules match (see **§2.5**, TECH-SPEC **§0.9**).

### 17.6 Schematic bank lanes (inner `01`, jetty name band)

On **Allocation → Jetty Schematic**, each configured jetty **top** or **bottom** cell is a **zone** containing: (1) a **jetty name band** showing the short **berth id** (e.g. **1A**, **1B**) flush against the central **pipeline** (black bar), plus an optional **CCTV camera** control when the user has **View Jetty Live stream** (`can_approve` on **At-Berth Executions**) and master data defines an RTSP link for that jetty (**§2.15**), and (2) **`capacity`** lane boxes. Each lane shows the **bank suffix** only (**`01`**, **`02`**, **`03`**, …); the full lane id **`{berthId}-NN`** appears on **hover** (e.g. tooltip **`1A-01`**). **Inner bank** is **`01`** (closest to the pipeline on **both** top and bottom); **`02`** is the next **outward**, **`03`** outward again for triple bank. The **top** lane stack uses reversed vertical order so **`01`** stays inner toward the pipeline (the **bottom** stack keeps natural order). Each box holds **at most one** displayed vessel (vacant otherwise), except **spanned** lanes which show a placeholder when another jetty’s occupant spans into this lane (**§2.28**). Occupants for the selected day are ordered like the Jetty schedule Gantt: **TB** ascending, then **operation id**, then **vessel id** (see TECH-SPEC **§0.6**, **§0.34**). If more occupied vessels than **capacity**, the last lane shows **+N more** after the representative vessel for that lane. **Incoming** names are hinted on the **first vacant** lane only (**§17.8**). When **`additionalJetties`** is set, the primary berth’s vessel graphic may **span horizontally** across adjacent columns.
...
- **Lane chrome:** **Vacant** lanes show the **bank suffix** (`01`, `02`, …) as a small **chip** in the lane’s **top-left** corner. **Occupied** lanes show the suffix inside the white info card **title row** (full `{berthId}-NN` remains on hover/tooltip).
- **Occupied lane composite:** Each occupied lane renders a **top-view vessel graphic** (SVG hull tinted by purpose) and a separate **white info card** (`.jetty-card__box`). **Top** berth zones (`1A`, `2A`, …) use **column-reverse** so the card sits **above** the graphic (inner / pipeline-adjacent); **bottom** zones (`1B`, `2B`, …) use normal column so the graphic sits **above** the card. Status badges (**Loading** / **Unloading**, **ETC breach**) live **inside** the info card only—not on the vessel graphic.
- **Readability:** Lane copy uses **compact** typography and padding. Info card body lines (**agent**, **cargo**, **balance**, **berthed**) use **`flex-shrink: 0`** so they do not compress or overlap when the card is shorter than its content. The card does **not** clip text (`overflow: visible`); when content exceeds the fixed lane height, the **lane** scrolls vertically (`overflow-y: auto` on `.jetty-schematic__lane`). **Plan ref** / **SI No** is **not** shown on the schematic card (available in lane tooltip and vessel modal).
...
| **ETC breach on schematic** | **ETC breached** card ring styling and an **icon-only** breach indicator (`EtcBreachBadge`) appear **only when View as of = today** (live operational signal). The icon is a **child of the white info card** — in the **title row**, immediately to the right of the **Loading** / **Unloading** purpose badge (`jetty-card__title-actions`) — so it stays on the card regardless of whether the vessel graphic is above or below the card (**top** vs **bottom** berth zones). Past/future dates do not show breach styling even if the vessel was past ETC on that day. |
| **Lane content (occupied)** | **Title row** (inside the white info card): bank suffix chip (`01`), **vessel name** (ellipsis when long), **purpose** badge (**Loading** / **Unloading**), and—when **View as of = today** and ETC is breached—the **icon-only** breach indicator. **Body lines:** optional **agent**; **material** + cargo progress; optional **balance**; optional **Berthed** duration (time since TB). **Plan ref** / **SI No** is omitted from the card to avoid overlap with the vessel graphic in fixed-height lanes; it remains on the lane **tooltip** and in the **Active vessel** modal. Data comes from Allocation **`vesselById`** when available; occupant fallback supports historical rows not in the live map. The **vessel graphic** is separate from the card and does not host status badges. |
