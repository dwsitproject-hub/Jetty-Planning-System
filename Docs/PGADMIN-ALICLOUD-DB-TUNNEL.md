...
| **Planned fix (when you continue)** | Publish JPS Postgres only on **loopback**: `127.0.0.1:<HOST_PORT>:5432` (repo default **HOST_PORT = 15432** in `docker-compose.backend.yml`). Postgres **inside** the container stays on **5432**; the **left** number is the port **on the Linux host** you forward through SSH. |
...
In the Compose file that defines **`jps-db`**, under that service, ensure you have (adjust **15432** if that port is busy on the host):
...
- **15432** = port on the **Linux host** (choose another free port if needed: `sudo ss -tlnp | grep 15432`).
...
Expect something like **`127.0.0.1:15432->5432/tcp`**.

### A.3 Test from the backend host

```bash
docker exec jps-db env | grep POSTGRES_
PGPASSWORD='<POSTGRES_PASSWORD>' psql -h 127.0.0.1 -p 15432 -U jps_user -d jps_db -c 'SELECT 1'
```
...
```bash
ssh -N -L 5433:127.0.0.1:15432 root@<BACKEND_HOST>
```

- Replace **15432** if you chose a different host port in Part A.
- Leave this window open while using pgAdmin.

**PuTTY:** Connection → SSH → Tunnels → **Source port** `5433`, **Destination** `127.0.0.1:15432`, **Local**, Add, then open the session to `<BACKEND_HOST>`.
...
```bash
ssh -N -L 5433:127.0.0.1:15432 root@<BACKEND_PRIVATE_IP> -J root@<APP_PUBLIC_IP>
```

**PuTTY:** Easiest is two sessions: first connect to the app server with a tunnel **to the backend’s SSH**, or use **Connection → Proxy** to send the main session through the app server, then add **Local** tunnel `5433` → `127.0.0.1:15432` on the **backend** side. (Exact clicks depend on your PuTTY version; the logical target is always **backend loopback:15432**.)
...
| **Port** | **5433** (your **local** tunnel port from Part B), **not** 15432 unless you intentionally use the same number locally |
...
1. [ ] On **backend**: `docker ps` shows **`jps-db`** with **`127.0.0.1:15432->5432`** (or your chosen host port).
2. [ ] On **backend**: `psql -h 127.0.0.1 -p 15432 ...` succeeds with **`jps_user`** / **`POSTGRES_PASSWORD`**.
3. [ ] On **PC**: SSH tunnel active, local port **5433** (or chosen) forwarding to **`127.0.0.1:15432`** on the backend.
...
The checked-in **`docker-compose.backend.yml`** at the project root includes the optional **`127.0.0.1:15432:5432`** mapping for **`jps-db`**. After `git pull` on the backend server, align the server’s compose file with this if you deploy from the same repo.
